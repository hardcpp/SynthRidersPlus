...
#pragma warning restore CS0649
        ...
        public async Task TryRequestResources(string p_ChannelID, string p_ChannelName, string p_AccessToken)
        {
            ...
            try
            {
                var l_URL = l_IsGlobal ? "https://api.7tv.app/v2/emotes/global" : $"https://api.7tv.app/v2/users/{p_ChannelID}/emotes";
                ChatPlexSDK.Logger.Debug($"Requesting 7TV Twitch {(l_IsGlobal ? "global " : "")}emotes{(l_IsGlobal ? "." : $" for channel {p_ChannelName}")}. " + l_URL);

                using (HttpRequestMessage msg = new HttpRequestMessage(HttpMethod.Get, l_URL))
                {
                    ...
                    if (l_Response.IsSuccessStatusCode)
                    {
                        ...
                        if (l_JSON.IsArray)
                        {
                            ...
                            foreach (JSONObject l_Object in l_JSON.AsArray)
                            {
                                ...
                                if (l_Object["id"].Value == "000000000000000000000000")
                                    continue;
                            ...
                            ChatPlexSDK.Logger.Debug($"Success caching {l_Count} 7TV {(l_IsGlobal ? "global " : "")}emotes{(l_IsGlobal ? "." : " for channel " + p_ChannelName)}.");
                        }
                        else
                            ChatPlexSDK.Logger.Error("emotes was not an array.");
                    }
                    else
                        ChatPlexSDK.Logger.Error($"Unsuccessful status code when requesting 7TV {(l_IsGlobal ? "global " : "")}emotes{(l_IsGlobal ? "." : " for channel " + p_ChannelName)}. {l_Response.ReasonPhrase}");
                }
            }
